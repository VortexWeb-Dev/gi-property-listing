...
<script>
    function prepareFilters(e) {
        ...
        const fieldMappings = {
            'city': '%ufCrm37City',
            'community': '%ufCrm37Community',
            'subCommunity': '%ufCrm37SubCommunity',
            'building': '%ufCrm37Tower',
            'reference': 'ufCrm37ReferenceNumber',
            'permit': 'ufCrm37ReraPermitNumber',
            'listing_title': '%ufCrm37TitleEn',
            'property_type': 'ufCrm37PropertyType',
            'saleRent': 'ufCrm37OfferingType',
            'listing_agent': '%ufCrm37AgentName',
            'developer': '%ufCrm37Developers',
            'listing_owner': '%ufCrm37ListingOwner',
            'landlordEmail': '%ufCrm37LandlordEmail',
            'landlordPhone': '%ufCrm37LandlordContact',
            'bedrooms': 'ufCrm37Bedroom',
            'bathrooms': 'ufCrm37Bathroom',
            'price': 'ufCrm37Price',
            'portal': 'portal'
        }
        ...
        if (filterParams['portal']) {
            if (filterParams['portal'] == 'PF') {
                filterParams['ufCrm37PfEnable'] = 1
            } else if (filterParams['portal'] == 'BAYUT') {
                filterParams['ufCrm37BayutEnable'] = 1
            } else if (filterParams['portal'] == 'DUBIZZLE') {
                filterParams['ufCrm37DubizzleEnable'] = 1
            } else if (filterParams['portal'] == 'WEBSITE') {
                filterParams['ufCrm37WebsiteEnable'] = 1
            }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const fetchAgents = async () => {
            try {
                const response = await fetch(`${baseUrl}/crm.item.list?entityTypeId=${AGENTS_ENTITY_ID}&select[0]=id&select[1]=ufCrm38AgentName&order[ufCrm38AgentName]=asc`);
