...
<script>
    ...
    async function fetchAgents(page = 1) {
        ...
        const apiUrl = `${baseUrl}/crm.item.list?entityTypeId=${entityTypeId}&order[id]=desc&select[0]=id&select[1]=ufCrm38AgentName&select[2]=ufCrm38AgentEmail&select[3]=ufCrm38AgentMobile&select[4]=ufCrm38AgentLicense&select[5]=ufCrm38AgentPhoto&select[6]=ufCrm38AgentId&start=${(page - 1) * pageSize}`;
        ...
        try {
            ...
            agentList.innerHTML = agents
                .map(
                    (agent) => `
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${agent.ufCrm38AgentId}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 flex items-center space-x-2">
                        <img 
                            class="w-10 h-10 object-cover rounded-full" 
                            src="${agent.ufCrm38AgentPhoto || './assets/images/agent-placeholder.webp'}" 
                            alt="${agent.ufCrm38AgentName || 'Agent'}" />
                        <span>${agent.ufCrm38AgentName || 'Agent'}</span>
                    </td>

                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${agent.ufCrm38AgentEmail || ''}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${agent.ufCrm38AgentMobile || ''}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${agent.ufCrm38AgentLicense || ''}</td>
